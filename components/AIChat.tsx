...
import { askGemini, getGeminiSpeech, playPcmAudio } from '../services/geminiService';
...
export const AIChat: React.FC<AIChatProps> = ({ 
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: '반가워요! StepCode AI 튜터입니다. 현재 배우고 있는 코드의 원리나 궁금한 점을 편하게 물어보세요!' }
  ]);
  ...
  const handleSend = async () => {
    ...
    const context = `
      [현재 학습 단계]: ${currentStage}
      [레슨]: ${currentLesson?.title || ''}
      [문제]: ${currentProblem?.question || ''}
    `;

    const response = await askGemini(input, context);
    setMessages(prev => [...prev, { role: 'assistant', content: response || '답변을 생성하지 못했습니다.' }]);
    setIsLoading(false);
  };
  ...
  if (isMinimized) {
    return (
      <div className="w-full h-full flex flex-col items-center py-8 gap-8 bg-[#0a0a0a]/80 backdrop-blur-xl">
        ...
        <button 
          onClick={onToggleMinimize}
          className="p-4 text-gray-500 hover:text-white transition-all bg-white/5 rounded-2xl hover:bg-white/10"
          title="대화창 확장"
        >
          <Maximize2 size={24} />
        </button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="[writing-mode:vertical-rl] rotate-180 text-gray-700 font-black uppercase tracking-[0.4em] text-[11px] py-10 select-none opacity-50">
            StepCode AI Assistant
          </div>
        </div>

        <div className="flex flex-col gap-4">
           <div className="w-1.5 h-1.5 rounded-full bg-[#007AFF] animate-pulse" />
           <div className="w-1.5 h-1.5 rounded-full bg-[#007AFF]/50 animate-pulse [animation-delay:0.2s]" />
           <div className="w-1.5 h-1.5 rounded-full bg-[#007AFF]/20 animate-pulse [animation-delay:0.4s]" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full flex flex-col bg-black/40">
      ...
      <div className="p-6 border-t border-white/5 bg-[#0a0a0a]/50">
        <div className="flex items-center gap-3 bg-white/[0.03] border border-white/10 rounded-[24px] pl-5 pr-3 py-3 focus-within:border-[#007AFF]/50 focus-within:bg-white/[0.05] transition-all group">
          <input 
            type="text" 
            value={input}
