...
export const Playground: React.FC<PlaygroundProps> = ({ user }) => {
  const [lang] = useState<Language>('python');
  ...
  const handleInputSubmit = async (e: React.FormEvent) => {
    ...
    if (lang === 'python' && inputResolveRef.current) {
      outputBufferRef.current += val + '\n';
    ...
    <div className="p-4 lg:p-12 h-full flex flex-col gap-4 lg:gap-8 max-w-[1600px] mx-auto overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4 lg:gap-6 w-full sm:w-auto">
          <div className="flex items-center gap-2 lg:gap-3 px-4 lg:px-6 py-2.5 lg:py-4 glass rounded-xl lg:rounded-2xl font-bold text-sm lg:text-base text-orange-accent">
            Python 3.12
          </div>
          <h2 className="text-xl lg:text-4xl font-bold tracking-tight truncate">연습장</h2>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
           <button onClick={() => { fetchSnippets(); setIsLoadModalOpen(true); }} className="p-2.5 lg:p-4 glass rounded-xl lg:rounded-2xl text-gray-500 hover:text-orange-accent transition-all shrink-0" title="불러오기"><FolderOpen size={18} /></button>
           ...
           {isExecuting ? (
             ...
             <button onClick={runCode} disabled={isEngineLoading} className="flex-1 sm:flex-none px-6 lg:px-10 py-2.5 lg:py-4 bg-orange-accent text-white rounded-xl lg:rounded-2xl font-black flex items-center justify-center gap-2 lg:gap-3 shadow-2xl active:scale-95 transition-all text-sm lg:text-base disabled:opacity-50">
      ...
      <div className="flex-1 flex flex-col xl:flex-row gap-4 lg:gap-8 min-h-0">
        <div className="flex-[2] flex flex-col gap-4 lg:gap-8 min-h-0">
          <div className="flex-1 glass rounded-[24px] lg:rounded-[40px] border-black/10 bg-white flex flex-col shadow-2xl overflow-hidden min-h-[300px]">
             <div className="px-5 py-3 lg:py-4 bg-black/[0.03] border-b border-black/5 flex items-center justify-between">
                ...
                <span className="text-[8px] lg:text-[10px] text-gray-500 font-mono uppercase tracking-widest">main.py</span>
             </div>
             <div className="flex-1 flex overflow-hidden relative font-mono text-sm lg:text-xl">
                <div ref={lineNumbersRef} className="w-10 lg:w-16 bg-black/5 border-r border-black/5 pt-6 lg:pt-10 flex flex-col items-stretch text-gray-500 select-none overflow-hidden">
                ...
                <textarea 
                  ...
                  className="flex-1 bg-transparent border-none outline-none text-main pt-6 lg:pt-10 px-4 lg:px-8 pb-10 resize-none leading-[24px] lg:leading-[32px] font-mono whitespace-pre custom-scrollbar" 
                  spellCheck={false} 
                />
              </div>
          </div>
        ...
        <div className="flex-1 glass rounded-[24px] lg:rounded-[40px] border-black/10 bg-white flex flex-col shadow-2xl overflow-hidden min-h-[200px] xl:min-h-0">
           <div className="px-5 py-3 lg:py-4 bg-black/[0.03] border-b border-black/5 flex items-center gap-3"><Terminal size={16} className="text-orange-accent" /><span className="text-[8px] lg:text-[10px] text-gray-500 font-bold uppercase tracking-widest">Output</span></div>
           <div className="flex-1 p-5 lg:p-8 font-mono text-xs lg:text-base overflow-y-auto bg-black/5 custom-scrollbar">
               {isEngineLoading ? (
                 ...
                 <div className="whitespace-pre-wrap text-teal-accent leading-relaxed">
                   {output}
                   {isWaitingForInput && (
                     <form onSubmit={handleInputSubmit} className="inline-flex items-center">
                       <div className="grid grid-cols-1 grid-rows-1 items-center">
                         ...
                         <input 
                           ...
                           className="row-start-1 col-start-1 bg-transparent border-none outline-none text-main font-mono p-0 m-0 w-full"
                         />
                       </div>
                     </form>
                   )}
      ...
      <AnimatePresence>
        {isSaveModalOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md glass p-8 rounded-[32px] border-black/10 bg-white shadow-2xl">
              <h3 className="text-2xl font-black text-main mb-6">코드 저장하기</h3>
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Snippet Title</label>
                  <input value={snippetTitle} onChange={e => setSnippetTitle(e.target.value)} placeholder="코드의 이름을 입력하세요" className="w-full bg-black/5 border border-black/10 rounded-xl p-4 text-sm text-main outline-none focus:border-orange-accent/50 transition-all" />
                </div>
                <button onClick={handleSaveSnippet} disabled={isSaving || !snippetTitle.trim()} className="w-full py-4 bg-orange-accent text-white rounded-xl font-black shadow-xl hover:scale-[1.02] active:scale-98 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <AnimatePresence>
        {isLoadModalOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-2xl glass p-8 rounded-[32px] border-black/10 bg-white shadow-2xl max-h-[80vh] flex flex-col">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-black text-main">저장된 코드 불러오기</h3>
                <button onClick={() => setIsLoadModalOpen(false)} className="p-2 text-gray-500 hover:text-main transition-all"><X size={24} /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
                {isLoadingSnippets ? (
                  ...
                  snippets.map(s => (
                    <div key={s.id} className="group flex items-center justify-between p-5 glass rounded-2xl border-black/5 hover:bg-black/5 transition-all">
                      <div className="cursor-pointer flex-1" onClick={() => handleLoadSnippet(s)}>
                        <h4 className="text-main font-bold mb-1">{s.title}</h4>
                        <div className="flex items-center gap-3">
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-400`}>{s.language}</span>
